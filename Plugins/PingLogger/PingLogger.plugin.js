...
class PingLogger {
  ...
  start() {
    console.log("[PingLogger] PingLogger loaded!");
    console.log("[PingLogger] PingLogger enabled!");
    ...
    this.patchModule = BdApi.Patcher.after("PingLogger", Dispatcher, "dispatch", (thisObject, args, returnValue) => {
      ...
      if (
        ...
        // Сохраняем уведомление с данными отправителя и данными аккаунта
        ...
        console.log(`[PingLogger] New ping from ${senderName} (Account: ${currentUserName})`);
      }
    });
  }
  stop() {
    BdApi.Patcher.unpatchAll("PingLogger");
    console.log("[PingLogger] PingLogger disabled!");
  }
  getSettingsPanel() {
    ...
    btnShowNotifications.addEventListener("click", () => {
        ...
        notificationsToShow.length === 0 ?
          ...
          notificationsToShow.map((notification, index) =>
            React.createElement(
              ...
              React.createElement("span", { style: { color: "#999999", fontSize: "11px", marginRight: "5px" }, title: (() => {
                    let now = new Date(notification.timestamp || Date.now());
                    ...
                    if (lang === "ru") {
                      ...
                      return `${pad(hours12)}:${pad(minutes)}:${pad(seconds)} ${pad(month)}.${pad(day)}.${year}`;
                    }
                  })() },
                  (() => {
                    let now = new Date(notification.timestamp || Date.now());
              ...
              React.createElement("span", { style: { color: "#7289da", cursor: "pointer", marginRight: "5px" }, title: i18n.copyTooltip, onClick: () => this.copyText(notification.authorId) }, notification.authorName),
      ...
      BdApi.showConfirmationModal(titleElement, modalContent, {
        ...
        onCancel: () => {
          this.notifications = [];
          BdApi.saveData("PingLogger", "notifications", this.notifications);
          console.log("[PingLogger] Notifications cleared!");
