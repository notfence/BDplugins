...
class PingLogger {
  ...
  resolveGuildNameSafely(guildId) {
    ...
    try {
      const GuildStore = BdApi.Webpack.getModuleByProps && BdApi.Webpack.getModuleByProps("getGuild", "getGuilds") || BdApi.Webpack.getModule(m => m && (m.getGuild || m.getGuilds));
      let guild = null;
      if (GuildStore) {
        try {
      ...
      if (guild) {
        const name = guild.name || guild.Name || guild.guildName || guild._name || guild.name_raw || (typeof guild.getName === 'function' ? (() => { try { return guild.getName(); } catch { return null; } })() : null);
  ...
  getDmLabelForLocale() { return i18n.dmLabel; }
  ...
  start() {
    ...
    const ChannelStore = (BdApi.Webpack.getStore ? BdApi.Webpack.getStore("ChannelStore") : (BdApi.Webpack.getModuleByProps ? BdApi.Webpack.getModuleByProps("getChannel") : null));
    ...
    this._closeOtherDialogs = () => {
      try {
        ...
        document.querySelectorAll('.layer-3JKvBn, .backdrop-1WG7uL, .modal-3zV5v4, .modal-2C4O6P, .backdrop-2kV7oH, .backdrop-2Gk4, .backdrop-2EY0G').forEach(el => { try { el.remove(); } catch {} });
    ...
    this.patchId = BdApi.Patcher.after("PingLogger", Dispatcher, "dispatch", (thisObject, args) => {
      try {
        ...
        try {
          ...
          if (Array.isArray(mentions) && mentions.length) isMentioned = mentions.some(m => { if (!m) return false; const mid = m.id ?? m.userId; return mid && String(mid) === currentUserId; });
        } catch {}
        if (!isMentioned) {
          try {
        ...
        try {
          ...
          if (this.fetchEveryoneAndRoles && Array.isArray(message.mention_roles) && message.mention_roles.length && guildId) {
            const MemberStore = BdApi.Webpack.getModuleByProps && BdApi.Webpack.getModuleByProps("getMember", "getMembers") || BdApi.Webpack.getModule(m => m && (m.getMember || m.getMembers));
            let member = null;
            try {
              if (MemberStore) {
        ...
        try { if (processedText) processedText = processedText.replace(/<@&\d+>/g, "@role"); } catch {}
        ...
        if (guildId) {
          ...
          this.notifications.push({ text: processedText, authorName: senderName, authorId: senderId, accountName: currentUserName, timestamp: Date.now(), serverName, guildId, channelId: channelId || null, messageId: safeString(message.id || message.message_id || null), channelName: null, isGroupDm: false });
        } else if (channelId) {
          try {
          ...
          this.notifications.push({ text: processedText, authorName: senderName, authorId: senderId, accountName: currentUserName, timestamp: Date.now(), serverName, guildId: null, channelId: channelId, messageId: safeString(message.id || message.message_id || null), channelName, isGroupDm });
        } else {
          this.notifications.push({ text: processedText, authorName: senderName, authorId: senderId, accountName: currentUserName, timestamp: Date.now(), serverName: this.getDmLabelForLocale(), guildId: null, channelId: null, messageId: safeString(message.id || message.message_id || null), channelName: null, isGroupDm: false });
        }
  ...
  _createLargeModal(notificationsToShow) {
    ...
    let closing = false;
    function removeModal() { if (closing) return; closing = true; const onFinish = () => { try { overlay.remove(); } catch {} document.removeEventListener("keydown", onKeyDown); }; try { modal.style.animation = ""; overlay.style.animation = ""; } catch {} modal.addEventListener("animationend", onFinish, { once: true }); overlay.classList.add('pinglogger-closing-overlay'); modal.classList.add('pinglogger-closing-modal'); setTimeout(onFinish, 700); }
